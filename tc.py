...
# ---------------------------
# Step 3: Load student info from CSV (Flexible headers)
# ---------------------------
student_info_map = {}
with open(STUDENT_INFO_FILE, newline='') as csvfile:
    ...
    for row in reader:
        ...
        # support both "standard" and "class"
        standard = row.get('standard', row.get('class', '')).strip()

        student_info_map[filename] = {
            "name": name,
            "roll_no": roll_no,
            "standard": standard
        }
    ...
    for filename in os.listdir(KNOWN_FACES_DIR):
        if filename.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            if len(encodings) == 0:
                ...
                known_names.append(info['name'] if info else os.path.splitext(filename)[0])
                print(f"[INFO] Added {filename} -> {known_names[-1]}")

                # Insert into MongoDB if not exists
                if info and students_collection.count_documents({"name": info['name']}) == 0:
...
# ---------------------------
# Step 5: Attendance Tracker
# ---------------------------
attendance_set = set()  # prevent duplicate entries in a session

def mark_attendance(name):
    """Mark attendance in MongoDB for recognized face."""
